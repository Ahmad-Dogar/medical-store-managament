  ...
  if (isset($_POST['updateBill'])) 
  {
    ...
    foreach ($_SESSION['bill'] as $row) 
    {
      ...
      echo "<td> 
            ...
            <input type='number' min='1' class='form-control input-sm pull-left' value ='$row[qty]' style='width:88px;' name='qty_$row[id]'>  <button type='submit' name='updateBill' style='margin-left:2px' class='btn btn-success btn-sm'>Update</button>
            </form>
            </td>";
      echo "</tr>";
      ...
      // $qry = "UPDATE inventeries SET `stock` = stock - $row[qty] WHERE `id` = $row[id]";
...
//       $id = $row['id'];
//       echo $id;
//       if(isset($_POST['qty_'.$id])){
        
//       $qty = $_POST['qty_'.$id];
      
//       $con=mysqli_connect('localhost','root','','store');  
//       $qry = "INSERT INTO get (item_id, qty)
//       VALUES ($id, $qty)";
//       $run=mysqli_query($con,$qry);
...
// }
  ...
  // print_r($_SESSION['array_qty']);

if (isset($_POST['billInfo'])) {
  foreach ($_SESSION['bill'] as $row) {
      $id = $row['id'];
        echo $id.'etstttinghhnsk';
      

      foreach ($qty_array as $qtyarr) {
        echo $qtyarr;
        $con=mysqli_connect('localhost','root','','store');  
        $qry = "UPDATE inventeries SET `stock` = stock - $qtyarr WHERE `id` = $id";
        $run=mysqli_query($con,$qry);
      }
